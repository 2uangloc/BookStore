...
namespace BookStoreWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            {
                // Người dùng chưa đăng nhập: chỉ được đăng ký Customer
                roleList = new List<SelectListItem>
                {
                    new SelectListItem { Text = SD.Role_Customer, Value = SD.Role_Customer }
                };
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Kiểm tra role vừa được gán thành công hay không
                    var roles = await _userManager.GetRolesAsync(user);
                    foreach (var role in roles)
                    {
                        _logger.LogInformation("✔ User vừa đăng ký thuộc role: " + role);
